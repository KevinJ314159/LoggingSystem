...
//    input CLK_10MHZ,            // 10MHz时钟输入
    ...
    input DownSig_RClk,         // 来自串并转换器的恢复时钟
    input DownSig_nLock,        // 来自串并转换器的锁定信号，正常工作时为低电平
    input [9:0] DownSig_Rout,   // 串并转换器输出的 10 位数据
    output sync_success,         // 同步成功标志
    output reg [9:0] DlDataOut,    // 给下行RAM的数据
    output reg DlDataOutEn      // 下行数据输出有效信号，在整个通信帧中拉高
);
...
/*    // 实例化edge_detect模块
    edge_detect edge_detect_instance (
        .DownSig_RClk(DownSig_RClk),    // 来自解串器的恢复时钟
...
//        .detected_negedge(negedge_detected), // 从edge_detect模块接收到的下降沿标志
        ...
        .DownSig_nLock(DownSig_nLock),   // 串并转换器的锁定信号
...
//        UlDataOutEn <= 1'b0;
        ...
        if (detect_flag) begin
            if (hold_counter < 6'd36) begin               // 一个通信帧38个字节
